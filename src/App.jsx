...
// Renommé en App pour l'export par défaut
const App = () => {
  ...
  const toggleTvMode = async (enable) => {
    if (enable) {
      setTvMode(true);
      try { if (!document.fullscreenElement) await document.documentElement.requestFullscreen(); } catch (err) { console.error("Fullscreen blocked or not supported", err); }
    } else {
      setTvMode(false);
  ...
  return (
    // CONTENEUR PRINCIPAL
    <div className={`h-screen w-full overflow-hidden flex flex-col font-sans select-none transition-colors duration-500 ${tvMode ? 'bg-black items-center justify-center' : 'bg-slate-950'}`}>
              ...
              {config.showFouls && (
                <div className="bg-slate-900/50 p-4 rounded-xl border border-slate-700">
                  <label className="block text-xs uppercase text-slate-400 font-bold mb-2 flex items-center gap-2">
                    <AlertCircle size={14} className="text-red-500" /> Alerte Fautes à :</label>
                  <div className="flex items-center gap-4 justify-center bg-slate-800 p-2 rounded border border-slate-600">
                ...
                >
                  <div className="flex flex-row items-center justify-center h-full w-full gap-4">
                    <span className={`text-xs md:text-base font-bold uppercase tracking-wider flex items-center gap-2 ${homeFouls >= config.foulLimit ? 'text-red-200' : 'text-slate-500'}`}>
                      Fautes {homeFouls >= config.foulLimit && <AlertCircle size={16} className="text-red-500" />}
                    </span>
                    <div className={`text-4xl md:text-6xl font-bold leading-none tabular-nums ${homeFouls >= config.foulLimit ? 'text-red-500' : 'text-white'}`}>{homeFouls}</div>
                  </div>
                </GestureArea>
              )}
            </div>
            ...
            <div className="w-full shrink-0 flex flex-col" style={{ height: '37%' }}>
              ...
              <div className="flex-1 w-full flex items-end justify-center px-6 pt-2 pb-2 md:px-10 md:pt-4 md:pb-2 overflow-hidden">
                {useAwayLogo && awayLogo ? (
                  ...
                  <h2 className="text-xl md:text-4xl font-black uppercase truncate tracking-tighter text-center w-full px-2 leading-snug" style={{ color: awayColor }}>{awayName}</h2>
                )}
              </div>
            </div>
                ...
                >
                  <div className="flex flex-row items-center justify-center h-full w-full gap-4">
                    <span className={`text-xs md:text-base font-bold uppercase tracking-wider flex items-center gap-2 ${awayFouls >= config.foulLimit ? 'text-red-200' : 'text-slate-500'}`}>
                      Fautes {awayFouls >= config.foulLimit && <AlertCircle size={16} className="text-red-500" />}
                    </span>
                    <div className={`text-4xl md:text-6xl font-bold leading-none tabular-nums ${awayFouls >= config.foulLimit ? 'text-red-500' : 'text-white'}`}>{awayFouls}</div>
                  </div>
                </GestureArea>
              )}
            </div>
